...
def main():
    ...
    if comm_rank ==0:
        ...
        for i in tqdm(range(max_generation)):
            ...
            antdict = antDict[1]
            while step < n:
                antdict.NeighborListACSDecisionRule(step)
                step += 1
            # 确保是走一个闭环，而不是非闭环
            antdict.tour.append(antdict.tour[0])
            antdict.computeTourLength() 
            # print(antdict.tour_length)
            # print(0,'down')     
            for cou in range(1,10):
                comm.send(antDict, dest=cou,tag=0)
            antDict[1] = antdict
        ...
        for i in (range(max_generation)):
            # print(comm_rank)
            antDict = comm.recv(source=0,tag = 0)
            # print(comm_rank,'recv')
            step = 1   
            antdict = antDict[comm_rank+1]
            while step < n:
                antdict.NeighborListACSDecisionRule(step)
                step += 1
            # 确保是走一个闭环，而不是非闭环
            antdict.tour.append(antDict[comm_rank].tour[0])
            antdict.computeTourLength()  
            comm.send(antdict, dest=0,tag = 1)  
            # if i != max_generation-1:
