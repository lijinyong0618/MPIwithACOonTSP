    ...
    if comm_rank ==0:
        ...
        for i in tqdm(range(max_generation)):
            ...
            for k in antDict.keys():
                ...
                antDict[k].tour.append(rand_node)
                antDict[k].visited[rand_node] = 1
            for cou in range(1, m):
                comm.send(antDict[cou + 1], dest=cou,tag=0)
            comm.bcast(pheromone, root=0)
            while step < n:
                antDict[1].NeighborListACSDecisionRule(step)
                step += 1
            # 确保是走一个闭环，而不是非闭环
            antDict[1].tour.append(antDict[1].tour[0])
            antDict[1].computeTourLength()
            # print(antdict[1].tour_length)
            # print(0,'down')
            # print(antDict[1].tour)
            for cou in range(1, m):
                antDict[cou + 1] = comm.recv(source=cou,tag=1)
                # print(antDict[cou].tour)
            ...
            #     comm.send(antDict, dest=cou,tag=2)
        plt.figure(2)
        ...
        for i in (range(max_generation)):
            # print(comm_rank)
            antDict[comm_rank + 1]= comm.recv(source=0,tag = 0)
            pheromone = comm.bcast(None, root=0)
            # print(comm_rank,'recv')
            step = 1
            while step < n:
                antDict[comm_rank + 1].NeighborListACSDecisionRule(step)
                step += 1
            # 确保是走一个闭环，而不是非闭环
            antDict[comm_rank + 1].tour.append(antDict[comm_rank + 1].tour[0])
            antDict[comm_rank + 1].computeTourLength()
            comm.send(antDict[comm_rank + 1], dest=0,tag = 1)
            # antDict = comm.recv(source=0,tag = 2)
            # best_k, history_best_tour_length, history_best_tour, history_best_generation = SaveInfo(i, history_best_tour_length, history_best_tour, history_best_generation)
            # ACSPheromoneUpdate(best_k,history_best_tour,history_best_tour_length)
main()